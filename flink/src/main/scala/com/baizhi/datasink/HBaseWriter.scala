...
class HBaseWriter extends RichSinkFunction[String]{
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    config.set(HConstants.ZOOKEEPER_QUORUM, "Spark")
    config.set(HConstants.ZOOKEEPER_CLIENT_PORT, "2181")
    config.setInt(HConstants.HBASE_CLIENT_OPERATION_TIMEOUT, 30000)
    config.setInt(HConstants.HBASE_CLIENT_SCANNER_TIMEOUT_PERIOD, 30000)
    conn = ConnectionFactory.createConnection(config)

    val tableName: TableName = TableName.valueOf("baizhi:t_user")
  ...
  override def invoke(value: String, context: SinkFunction.Context[_]): Unit = {
    ...
    put.addColumn(Bytes.toBytes(cf1), Bytes.toBytes("word"), Bytes.toBytes(array(1)))
    put.addColumn(Bytes.toBytes(cf1), Bytes.toBytes("num"), Bytes.toBytes(array(2)))
//    mutator.mutate(put)
//    //每满2000条刷新一下数据
//    if (count >= 2000){
//      mutator.flush()
